...
- [MONITORING] 高度阈值现象（现场新增）：
  - 蓝色环：相机高度 <= 15cm 时可稳定识别，> 15cm 基本识别不到。
  - 红色环：相机高度 <= 13cm 左右可稳定识别，> 13cm 识别率突降。
  - 绿色环：延续历史问题，整体仍偏难检出。
- [MONITORING] test_windows 方案复测现象：
  - 蓝色环在约 16cm 高度仍有约 50% 检出率。
  - 红色环突变高度约 12cm。
  - 绿色环仍为难点，检出稳定性不足。
...
- 改动内容：
  ...
  - 新增现象：
    - 蓝色环在 15cm 高度以下稳定识别，以上基本识别不到。
    - 红色环在 13cm 左右以下较稳定，超过后识别率明显突降。
    - 绿色环依旧是历史难点，整体检出偏弱。
- 踩到的坑：
  - 当前参数对“高度/尺度变化”敏感，识别存在明显分段突变而非平滑退化。
  - 绿色环在现有光照与阈值组合下仍缺乏足够可用候选。
- 解决方法：
  - 本轮按你的要求先记录现象，不做算法与参数改动。
  - 后续按“高度分段”建立验证基线（<=13cm、13-15cm、>15cm）再做定向调参。
- 验证结果：
...
- 改动内容：
  ...
  - 新增现象：
    ...
    - 绿色环仍较难稳定检出。
- 踩到的坑：
  - 同为高度变化场景，不同脚本（new_test 与 test_windows）表现阈值不一致，容易造成结论混淆。
- 解决方法：
  - 明确将本条结论标注为“test_windows 方案”，与其他脚本观测分开记录。
  - 本轮按要求只补录现象，不做算法和参数改动。
- 验证结果：
